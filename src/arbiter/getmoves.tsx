  ...
  if (prevPosition) {
    ...
    if ((dir === 1 && rank === 4) || (dir === -1 && rank === 3)) {
      adjacentFiles.forEach((f) => {
        if (
          position?.[rank]?.[f] === enemyPawn && //  checks if there is a pawn adjacent to the advancing pawn
          ...
          // checks if defending pawn was 2 blocks before its current position
...
export const getCastleDirections = ({ castleDirection, piece, file, rank }) => {
  file = Number(file);
  rank = Number(rank);
